...
import { UserInfo, UserTableProps } from "../../types/interfaces";
...
const UserTable: React.FC<UserTableProps> = ({ info }) => {
  ...
  const filteredInfo = info.filter((item: UserInfo) => {
  ...
  const deleteDocument = async () => {
    try {
      if (documentIdToDelete) {
        await deleteDoc(doc(db, "userDownloads", documentIdToDelete));
    ...
    <>
      ...
      <div className=" relative overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {filteredInfo.map((item: UserInfo, index: number) => (
